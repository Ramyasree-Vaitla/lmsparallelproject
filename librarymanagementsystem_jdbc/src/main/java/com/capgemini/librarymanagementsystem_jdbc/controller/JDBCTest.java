...
import com.capgemini.librarymanagementsystem_jdbc.validation.JDBCValidations;
...
public class JDBCTest {
	...
	public static void doReg() {
		...
		JDBCValidations validation = new JDBCValidations();
		do {
			try (Scanner scanner = new Scanner(System.in);) {
				System.out.println("****LIBRARY MANAGEMENT SYSTEM****");
				System.out.println("Press 1 to RegisterUser");
				System.out.println("Press 2 to LoginUser");
									...
									do {
										try {
											...
											case 4:
												...
												try {
													List<BookInfoBean> bookauthor = service2.searchBookByAuthor(author);
														...
														for (BookInfoBean bookBean : bookauthor) {
															...
															if (bookBean != null) {
																System.out.println(String.format(
																		...
																		bookBean.getBookName(), bookBean.getAuthor(),
											...
											case 9:
												...
												try {
													List<BookIssueInfoBean> uid = service3.bookHistoryDetails(user_Id);
													for (BookIssueInfoBean issueDetails : uid) {
														if (issueDetails != null) {
															System.out.println(
																	"-----------------------------------------------");
															System.out.println(
																	"No of books Borrowed :" + issueDetails.getuId());
														} else {
															System.out.println(
																	"-----------------------------------------------");
															System.out.println(
											...
											case 10:
												...
												try {
													List<RequestBookInfoBean> requests = service1.showRequests();
													System.out.println(String.format("%-10s %-25s %5s %s", "User_Id",
															"User_Name", "Book_Id", "BookName"));
													for (RequestBookInfoBean requestBean : requests) {
														if (requestBean != null) {
											...
											case 11:
												...
												try {
													...
													System.out.println(String.format("%-10s %-10s %15s %s", "Book_Id",
															"User_Id", "Issue_Date", "Return_Date"));
													for (BookIssueInfoBean issueBean : issuedBooks) {
														if (issueBean != null) {
															System.out.println(String.format("%-10s %-10s %15s %s",
																	issueBean.getbId(), issueBean.getuId(),
											...
											case 2:
												...
												try {
													if (loginInfo.getuId() == user_Id) {
														List<BoorowedBooksInfoBean> borrowedBookList = service3
																.borrowedBook(user_Id);
														System.out.println(String.format("%-5s %-5s %s", "User_Id",
																"Book_Id", "Email"));
														for (BoorowedBooksInfoBean bookBean : borrowedBookList) {

															System.out.println(
																	String.format("%-5s %-5s %s", bookBean.getuId(),
