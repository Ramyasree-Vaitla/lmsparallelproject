...
public class LMSUsersDAOImplement implements LMSUsersDAO {
				...
				if(rs1 != null) {
					...
					if(daysBetween>7.0) {
						...
						if(status=="yes") {
							...
							LMSBorrowedDetails bbb = (LMSBorrowedDetails) query3.getSingleResult();
							...
							LMSRequestDetails rdb = (LMSRequestDetails) query4.getSingleResult();
	...
	public List<Integer> bookHistoryDetails(int uId) {
		...
		TypedQuery<LMSBookIssueDetails> query = manager.createQuery(jpql,LMSBookIssueDetails.class);
		List<LMSBookIssueDetails> recordList = query.getResultList();
		for(LMSBookIssueDetails p : recordList) {
			noOfBooks = count++;
		}
	...
	public List<LMSRequestDetails> showRequests() {
		...
		TypedQuery<LMSRequestDetails> query = manager.createQuery(jpql,LMSRequestDetails.class);
		List<LMSRequestDetails> recordList = query.getResultList();
		manager.close();
		factory.close();
		return recordList;
	}
	...
	public List<LMSUsersDetails> showUsers() {
		...
		TypedQuery<LMSUsersDetails> query = manager.createQuery(jpql,LMSUsersDetails.class);
		List<LMSUsersDetails> recordList = query.getResultList();
		manager.close();
		factory.close();
		return recordList;
	}
	...
	public boolean updatePassword(int id, String password, String newPassword, String role) {
		try(FileInputStream info = new FileInputStream("db.properties");) {
			...
			query.setParameter("password", password);
			LMSUsersDetails rs = query.getSingleResult();
			if(rs != null) {
				LMSUsersDetails record = manager.find(LMSUsersDetails.class,id);
